				...
				<form>
					<div>
						...
						<h1 class="text-2xl font-bold">Login to your account</h1>
							...
							<input class="px-4 w-full border-2 py-2 rounded-md text-sm outline-none" type="text" v-model="username" name="username" placeholder="username">
        </div>
					<div class="my-3">
						<label class="block text-md mb-2" for="password">Password</label>
						<input class="px-4 w-full border-2 py-2 rounded-md text-sm outline-none" type="password" v-model="password" name="password" placeholder="password">
        </div>
							...
							<div class="">
								<button class="mt-4 mb-3 w-full bg-green-500 hover:bg-green-400 text-white py-2 rounded-md transition duration-100" @click.prevent="login">Login now</button>
...
import { ILoginResponse } from '@/types/apiTypes';
 ...
 function login (event:MouseEvent):void {
   ...
   password:password.value
    ...
    api.postRequest("/auth/login", userData)
    }
 }
</script>